...
class SimpleTelegramBot:
    ...
    def process_leverage_input(self, text):
        ...
        try:
            ...
            self.send_trading_setup_step4_capital('futures', leverage)
    ...
    def process_capital_input(self, text):
        ...
        try:
            ...
            # Sermaye doğrulaması
            is_valid, error_message = self.validate_capital_amount(capital, self.setup_data.get('exchange_type', 'spot'))
            
            if not is_valid:
                self.send_telegram_message(error_message)
                self.send_telegram_message("Lütfen tekrar deneyin:")
                return
            
            self.setup_data['capital'] = capital
            self.waiting_for_input = None
            self.send_trading_setup_step5_mode(
            ...
            # Bakiye kontrolü yap
            try:
                wallet_balance = self.get_wallet_balance(self.setup_data.get('exchange_type', 'spot'))
                if wallet_balance > 0:
                    self.send_telegram_message(f"❌ Geçersiz sermaye miktarı. Mevcut bakiyeniz: ${wallet_balance:,.2f}")
                    self.send_telegram_message("Lütfen geçerli bir sayı girin (örnek: 1000):")
                else:
                    self.send_telegram_message("❌ Geçersiz sermaye miktarı. Lütfen sadece sayı girin (örnek: 1000):")
            except:
                self.send_telegram_message("❌ Geçersiz sermaye miktarı. Lütfen sadece sayı girin (örnek: 1000):")

    def validate_capital_amount(self, capital, exchange_type):
        """Sermaye miktarını doğrula"""
        try:
            wallet_balance = self.get_wallet_balance(exchange_type)
            
            if capital > wallet_balance:
                return False, f"❌ Yetersiz bakiye! Mevcut bakiye: ${wallet_balance:,.2f} USDT"
            elif capital <= 0:
                return False, "❌ Sermaye 0'dan büyük olmalıdır."
            else:
                return True, ""
                
        except Exception as e:
            return True, ""  # Hata durumunda doğrulama atlansın

    def apply_settings_and_start(self):
        """Ayarları uygula ve trading'i başlat"""
        try:
            # Ayarları uygula
            self.timeframe = self.setup_data['timeframe']  # Yeni timeframe ayarı
...
📊 <b>Ayarlarınız:</b>
⏰ Zaman Dilimi: {timeframe_display[self.timeframe]}
...
⏰ Başlatma: {datetime.now().strftime('%H:%M:%S')}
    ...
    def process_telegram_callback(self, callback_query):
        ...
        # Setup adımları
            ...
            self.setup_data = {'timeframe': timeframe}
            self.send_trading_setup_step2_exchange()
        elif data == 'setup_spot':
            self.setup_data['exchange_type'] = 'spot'
            self.setup_data['leverage'] = 1
            self.send_trading_setup_step4_capital('spot', 1)
        elif data == 'setup_futures':
            self.setup_data['exchange_type'] = 'futures'
            self.send_trading_setup_step3_leverage()
        
        # Sermaye seçimi
        elif data.startswith('capital_'):
            if data == 'capital_custom':
                self.waiting_for_input = 'capital'
                self.send_telegram_message("""
💸 <b>Özel Miktar</b>

Lütfen istediğiniz sermaye miktarını yazın:
Örnek: 15000

Not: Mesajınızı sadece sayı olarak yazın.
                """)
                return
        ...
        # Geri butonları
            ...
            if self.setup_data.get('exchange_type') == 'futures':
                ...
                self.send_trading_setup_step2_exchange()
        elif data == 'back_step4':
            self.send_trading_setup_step4_capital(
                self.setup_data['exchange_type'],
                self.setup_data['leverage']
            )
...
⏰ <b>Zaman Dilimi:</b> {self.timeframe}
...
⏰ <b>Son Güncelleme:</b> {datetime.now().strftime('%H:%M:%S')}
    ...
    def send_trading_setup_step1(self):
        """Step 1: Zaman dilimi seç"""
        keyboard = self.create_keyboard([
            [
                {'text': '📊 15 Dakika', 'callback_data': 'timeframe_15m'},
                {'text': '📈 1 Saat', 'callback_data': 'timeframe_1h'}
            ],
            [
                {'text': '📉 4 Saat', 'callback_data': 'timeframe_4h'},
                {'text': '📅 1 Gün', 'callback_data': 'timeframe_1d'}
            ],
            [
        ...
        message = """
⚙️ <b>Trading Setup - Adım 1/5</b>

<b>Hangi zaman diliminde analiz yapmak istiyorsunuz?</b>
...
Hangi zaman dilimini tercih ediyorsunuz?
        """
        
        self.send_telegram_message(message, keyboard)

    def send_trading_setup_step2_exchange(self):
        """Step 2: Exchange tipi seç"""
        keyboard = self.create_keyboard([
            [
        ...
        current_timeframe = self.setup_data.get('timeframe', '15m')
        
        message = f"""
⚙️ <b>Trading Setup - Adım 2/5</b>

<b>Exchange tipini seçin:</b>

📊 <b>Seçiminiz:</b> {timeframe_display[current_timeframe]}
...
Hangi tipte trading yapmak istiyorsunuz?
    ...
    def send_trading_setup_step3_leverage(self):
        """Step 3: Leverage değeri gir (manuel)"""
        keyboard = self.create_keyboard([
            [
                {'text': '🔙 Geri', 'callback_data': 'back_step2'}
            ]
        ])
        ...
        message = f"""
⚙️ <b>Trading Setup - Adım 3/5</b>

<b>Leverage oranını yazın:</b>

📊 <b>Ayarlarınız:</b>
• Zaman Dilimi: {timeframe_display[current_timeframe]}
• Exchange: Futures
...
Minimum: 1x, Maksimum: 125x
    ...
    def send_trading_setup_step4_capital(self, exchange_type, leverage=1):
        """Step 4: Sermaye miktarı gir (Binance bakiyesinden kontrol)"""
        try:
            # Binance'dan bakiye bilgilerini al
            if exchange_type == 'spot':
                # Spot bakiyesi
                balance_info = self.exchange.fetch_balance()
                usdt_balance = balance_info['USDT']['free']
                wallet_type = "Spot Cüzdan"
            else:
                # Futures bakiyesi için exchange'i geçici olarak futures'a çevir
                ...
                balance_info = temp_exchange.fetch_balance()
                usdt_balance = balance_info['USDT']['free']
                wallet_type = "Futures Cüzdan"
            
            # Bakiye yoksa uyarı ver
            if usdt_balance <= 0:
                keyboard = self.create_keyboard([
                    [
                        {'text': '🔙 Geri', 'callback_data': 'back_step3' if exchange_type == 'futures' else 'back_step2'}
                    ]
                ])
...
{wallet_type}'ınızda USDT yok.

💰 <b>Mevcut Bakiye:</b> ${usdt_balance:.2f} USDT

Lütfen önce {wallet_type}'ınıza USDT yatırın.
                """
                
                self.send_telegram_message(message, keyboard)
                return
            
            # Manuel sermaye girişi için input bekle
            self.waiting_for_input = 'capital'
            ...
            message = f"""
⚙️ <b>Trading Setup - Adım {4 if exchange_type == 'spot' else 4}/5</b>
...
💰 <b>{wallet_type} Bakiyeniz:</b> ${usdt_balance:,.2f} USDT
...
💡 <b>Önerilen:</b> Tüm bakiyenizi kullanmayın, risk yönetimi yapın!

<b>Lütfen sermaye miktarını yazın (örnek: 1000):</b>
• Maksimum: ${usdt_balance:,.2f} USDT
            """
            
            self.send_telegram_message(message, keyboard)
            
        except Exception as e:
            self.send_telegram_message(f"❌ Bakiye bilgisi alınamadı: {e}")
            ...
            message = f"""
⚠️ <b>Bakiye alınamadı, manuel giriş yapın:</b>
...
Not: Sadece sayı yazın, $ işareti koymayın
    ...
    def send_trading_setup_step5_mode(self, exchange_type, leverage, capital):
        """Step 5: Trading modu seç"""
        keyboard = self.create_keyboard([
            [
            ...
            [
                {'text': '🔙 Geri', 'callback_data': 'back_step4'}
            ]
        ])
...
⏰ <b>Zaman Dilimi:</b> {timeframe_display[current_timeframe]}
...
⚖️ <b>Risk:</b> %2 per trade
    ...
    def get_wallet_balance(self, exchange_type='spot'):
        """Belirtilen cüzdan tipinden USDT bakiyesini al"""
        try:
            if exchange_type == 'spot':
                # Mevcut exchange spot olmalı
                ...
                # Futures için geçici exchange oluştur
                temp_exchange = ccxt.binance({
                    'apiKey': self.config['api_key'],
                    'secret': self.config['secret'],
                    'sandbox': self.config.get('sandbox', True),
            ...
            self.logger.error(f"Bakiye alınamadı ({exchange_type}): {e}")
            return 0
        """Trend analizi"""
        if len(prices) < 3:
