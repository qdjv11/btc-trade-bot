...
class SimpleTelegramBot:
    ...
    def process_telegram_command(self, message):
        ...
        # Özel miktar girişi kontrolü
        if text.isdigit() and hasattr(self, 'setup_data') and 'capital' not in self.setup_data:
            try:
                capital = int(text)
                if capital >= 100:  # Minimum 100$
                    ...
                    self.send_telegram_message("❌ Minimum sermaye $100 olmalıdır. Lütfen tekrar deneyin:")
            except:
                self.send_telegram_message("❌ Geçersiz miktar. Lütfen sadece sayı girin:")
            return
        ...
        elif text == '/report' or text == 'report':
            self.send_hourly_report()
        elif text == '/trading start' or text == 'trading start':
            self.start_trading()
        elif text == '/trading stop' or text == 'trading stop':
            self.stop_trading()
...
Hangi tipte trading yapmak istiyorsunuz?
    ...
    def send_trading_setup_step2_leverage(self):
        """Step 2: Leverage seç (sadece futures için)"""
        keyboard = self.create_keyboard([
            [
                {'text': '5x', 'callback_data': 'leverage_5'},
                {'text': '10x', 'callback_data': 'leverage_10'}
            ],
            [
                {'text': '20x', 'callback_data': 'leverage_20'},
                {'text': '25x', 'callback_data': 'leverage_25'}
            ],
            [
...
Kaç kat leverage kullanmak istiyorsunuz?
        """
        
        self.send_telegram_message(message, keyboard)

    def send_trading_setup_step3_capital(self, exchange_type, leverage=1):
        """Step 3: Sermaye miktarı seç"""
        keyboard = self.create_keyboard([
            [
                {'text': '$1,000', 'callback_data': 'capital_1000'},
                {'text': '$5,000', 'callback_data': 'capital_5000'}
            ],
            [
                {'text': '$10,000', 'callback_data': 'capital_10000'},
                {'text': '$25,000', 'callback_data': 'capital_25000'}
            ],
            [
                {'text': '$50,000', 'callback_data': 'capital_50000'},
                {'text': '💸 Özel Miktar', 'callback_data': 'capital_custom'}
            ],
            [
...
💡 <b>Önerilen:</b> İlk denemeler için $1,000-5,000
        """
